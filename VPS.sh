#!/bin/bash
# Do not edit this script by your own, this script is perfect to be run, and used. Thank you!
set -euo pipefail
...
cleanup() {
    ...
    if [ -f "$script_dir/user-data" ]; then rm -f "$script_dir/user-data"; fi
    if [ -f "$script_dir/meta-data" ]; then rm -f "$script_dir/meta-data"; fi
}
...
EOF
    ...
    if ! cloud-localds "$SEED_FILE" "$script_dir/user-data" "$script_dir/meta-data"; then
...
start_vm() {
    ...
    if load_vm_config "$vm_name"; then
        ...
        # Fixed: use dedicated counter for port forward netdev IDs
        if [[ -n "$PORT_FORWARDS" ]]; then
            local pf_index=1
            IFS=',' read -ra forwards <<< "$PORT_FORWARDS"
            for forward in "${forwards[@]}"; do
                IFS=':' read -r host_port guest_port <<< "$forward"
                qemu_cmd+=(-device "virtio-net-pci,netdev=pf${pf_index}")
                qemu_cmd+=(-netdev "user,id=pf${pf_index},hostfwd=tcp::${host_port}-:${guest_port}")
                ((pf_index++))
            done
        fi
...
# Fixed: use basename to avoid issues with special chars in path
is_vm_running() {
    local vm_name=$1
    local config_file="$VM_DIR/$vm_name.conf"
    if [[ -f "$config_file" ]]; then
        local img
        img=$(grep '^IMG_FILE=' "$config_file" | cut -d'"' -f2)
...
stop_vm() {
    ...
    if load_vm_config "$vm_name"; then
        if is_vm_running "$vm_name"; then
            ...
            pkill -f "qemu-system-x86_64.*${img_basename}" || true
            sleep 2
            if is_vm_running "$vm_name"; then
                print_status "WARN" "VM did not stop gracefully, forcing termination..."
                pkill -9 -f "qemu-system-x86_64.*${img_basename}" || true
            fi
        ...
        while true; do
            ...
            case $edit_choice in
                ...
                8)
                    ...
                    ;;
                9)
                    # Fixed: actually resize the image, not just update the variable
                    while true; do
...
show_vm_performance() {
    ...
    if load_vm_config "$vm_name"; then
        if is_vm_running "$vm_name"; then
            ...
            img_basename=$(basename "$IMG_FILE")
            local qemu_pid
            qemu_pid=$(pgrep -f "qemu-system-x86_64.*${img_basename}" | head -1)
            if [[ -n "$qemu_pid" ]]; then
